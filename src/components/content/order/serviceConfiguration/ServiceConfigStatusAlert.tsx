...
function ServiceConfigStatusAlert({
        ...
        if (updateConfigRequest.isPending) {
            ...
            if (
                ...
                if (serviceHostType === serviceHostingType.SERVICE_VENDOR) {
                    return 'Updated status polling failed. Please visit MyServices page to check the status of the request and contact service vendor for error details.';
                } else {
                    return 'Updated status polling failed. Please visit MyServices page to check the status of the request';
                }
            } else if (
    ...
    const getServiceId = (): string => {
        if (updateConfigRequest.isSuccess) {
            return updateConfigRequest.data.serviceId;
        } else {
            if (
                isHandleKnownErrorResponse(updateConfigRequest.error) &&
                'serviceId' in updateConfigRequest.error.body
            ) {
                return updateConfigRequest.error.body.serviceId as string;
            } else {
                return serviceId;
            }
        }
    ...
    return <UpdateConfigResult msg={msg ?? ''} uuid={getServiceId()} type={alertType} />;
}
